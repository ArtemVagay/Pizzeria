...
def buy(_value, cost):
    ...
    pdf.cell(0, 10, txt=f"Всего...{cost}р", ln = 1)
...
def user_order(value):
    ...
    if value[0]["shop_cart"] != []:
        ...
        buy_button = CTk.CTkButton(app, width=140, height=28, text="Заказать", fg_color="#24D418", hover_color="#0B870D", font = CTk.CTkFont("Comfortaa", size = 10, weight="bold"), command=lambda: buy(value, cost=global_cost))
        buy_button.pack()
